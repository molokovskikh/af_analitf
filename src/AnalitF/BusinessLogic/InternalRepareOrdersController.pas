...
begin
  for I := 0 to _infos.Count-1 do begin
    ...
    for orderIndex := 0 to info.Orders.Count-1 do begin
      ...
      try
        //ѕроизводим восстановление заказа при существовании предложений
        ...
        if offers.Count = 0 then begin
          freezingReason := IfThen(order.OrderItems.Count = 0, 'прайс-лист отсутствует', 'предложени€ отсутствуют');
          WriteExchangeLog('¬осстановление заказа', '«аказ ' + order.ToString() + ' был заморожен, т.к. ' + freezingReason);
          order.ErrorReason := '«аказ был заморожен, т.к. ' + freezingReason;
          DM.adcUpdate.SQL.Text := ''
      +' update '
      +'   CurrentOrderHeads '
      +'  set '
