...
type
  { ласс дл€ корректировки WindowProc всех ToughDBGrid на дочерней форме,
   чтобы у каждого грида была сво€ собственна€ нова€ WindowProc.
   ≈сли этого не сделать, то будет возникать ошибка "A call to an OS function failed"
   –ешение приведено на странице: http://www.delphikingdom.com/asp/answer.asp?IDAnswer=34018
   ѕроблема св€зана с тем, что не происходит восстановление фокуса у DB-компонент после того,
  ...
  protected
    ...
    // орректируем connection дл€ MySQL-компонент
    procedure PatchMyDataSets;
    //ѕровер€ем, что заказ сделан кратно Volume
    function  CheckVolume : Boolean;
    //ѕровер€ем, что заказ сделан >= OrderCost
  ...
  public
    ...
    //–азрешено сохран€ть отображаемую таблицу
    SaveEnabled: Boolean;
    //“ребуетс€ вызвать First после сортировки DataSet
...
begin
  inherited Loaded;
  //родителем будет главна€ форма
  Parent:=Application.MainForm;
  UpdateReclame;
end;

//дл€ того, чтобы TAction, созданные на дочерней форме работали, надо вставить
//их на главную форму
...
begin
  ...
  //ѕроверка на максимальную сумму заказа, выставл€емую сервером
...
begin
  ...
  //Ёто хак: чтобы метод не отрабатывал на adsCore из формы TOrdersForm  
...
begin
  if (stUpdate in StatementTypes) or (stRefresh in StatementTypes) then
    //¬озможна ситуаци€, когда параметра "ClientId" не будет в выполн€емой команде
...
begin
  ...
  if Assigned(realCostColumn) then  begin
    ...
    if not Assigned(supplierPriceMarkupColumn) then begin
      ...
      supplierPriceMarkupColumn.Title.Caption := 'Ќаценка поставщика';
      supplierPriceMarkupColumn.DisplayFormat := '0.00;;''''';
    end;
    //удал€ем столбец "÷ена без отсрочки", если не включен механизм с отсрочкой платежа
