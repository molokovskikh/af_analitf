...
begin
  if (cbRas.ItemIndex>=0)and(AProc.MessageBox('”далить удаленное соединение?',
...
begin
  AProc.MessageBox('»зменени€ вступ€т в силу после получени€ данных', MB_OK or MB_ICONWARNING);
end;
...
begin
  if (ModalResult = mrOK) then begin
    if HTTPNameChanged and (OldHTTPName <> dbeHTTPName.Field.AsString) then begin
      if AProc.MessageBox('»зменение имени авторизации удалит все неотправленные заказы. ѕродолжить?' , MB_ICONQUESTION or MB_YESNO) <> IDYES then
        CanClose := False;
    end;
    if CanClose and RetMarginsChanges then begin
      ...
      if mdRetail.RecordCount > 0 then begin
        ...
        if not Res then begin
          ...
          AProc.MessageBox('Ќекорректно введены границы цен.', MB_ICONWARNING);
        end;
      end;
    end;
  end;
