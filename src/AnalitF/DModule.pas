...
{$endif}
    ...
    //Подготовка директорий старой libmysqld к обновлению на новую CryptDLL после версии 1651
...
begin
  ...
  for I := 0 to fileList.Count-1 do begin
    ...
    if (openedFiles >= 10) and (i < fileList.Count-1) then begin
      AProc.MessageBox(
        Format('Автоматически открыты первые %d файлов вложений мини-почты.'#13#10'Остальные файлы можно открыть из писем мини-почты.', [openedFiles]),
        MB_ICONINFORMATION);
      Break;
    end;
  end;
