  ...
  try
    ...
    try
      ...
      if requestAttachs then begin
        WriteExchangeLog('Exchange', 'Количество в списоке запрашиваемых вложений: ' + IntToStr(AttachmentIdsSL.Count));
        for I := 0 to AttachmentIdsSL.Count-1 do
          AddPostParam('AttachmentIds', AttachmentIdsSL[i]);
      end
      else begin
        WriteExchangeLog('Exchange', 'Список запрашиваемых вложений - пуст!');
...
begin
  try
    ...
    try
      ...
      try
        if absentQuery.RecordCount > 0 then begin
          WriteExchangeLog('Exchange', 'Количество в результате запроса из базы запрашиваемых вложений: ' + IntToStr(absentQuery.RecordCount));
          ...
          WriteExchangeLog('Exchange', 'Результат запроса из базы списка запрашиваемых вложений - пустой!');
      finally
        absentQuery.Close;
      end;
    finally
