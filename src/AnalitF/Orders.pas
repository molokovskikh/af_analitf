    ...
    if Key = VK_ESCAPE then begin
      ...
      if (Key = VK_DELETE) then begin
        if (Sender = dbgEditOrders) then
          ...
          //Если таблица не dbgEditOrders, то это dbgOrders, что она не отрабатывала
          //нажатие клавиши Delete и выводила "Удалить значение", то убираем это нажатие
          Key := 0;
          if not ClosedOrder then
            //Удаляем позицию только из текущих заказов
            DeleteOrderPosition;
        end;
      end;
end;
