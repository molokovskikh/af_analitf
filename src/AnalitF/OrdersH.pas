...
begin
  ...
  if sp.PriceCode = Grid.DataSource.DataSet.FieldByName('PriceCode').AsInteger then
    AProc.MessageBox( 'Нельзя переместить заявку в тот же прайс-лист!', MB_ICONWARNING)
  else
  if AProc.MessageBox( 'Переместить выбранную заявку в прайс-лист ' + sp.PriceName + '?', MB_ICONQUESTION or MB_OKCANCEL) = IDOK
  then begin
    ...
    try
      ShowSQLWaiting(
        ...
        'Перемещение заявки в прайс ' + InternalDestinationPrice.PriceName);
