...
type
  ...
  //Раньше этот класс назывался TFakeDLL, теперь мы его переимновали в MDLHelper:
  //Memory DLL Load Helper
  TMDLHelper = class(TObject)
  constructor Create;
...
function TMDLHelper.GetFDProcAddress(ProcName :PChar) :Pointer;
var
  ProcAddr :Pointer;
begin
...
procedure TMDLHelper.InjectDll(MemoryFile :Pointer; FreeOld :LongBool);
begin
...
function TMDLHelper.ProcessDll(MemoryFile :Pointer) :Pointer;
var
  pMap :Pointer;
begin
  Result  := nil;
...
procedure TMDLHelper.FlushDll;
begin
  _DG_UnMapPeFile4(Pointer(FDHandle));
  FDHandle := 0;
end;

procedure TMDLHelper.ShowError;
var
  Buff :PChar;
begin
