/******************************************************************************/
/***          Generated by IBExpert 2005.06.21 04.08.2005 10:07:25          ***/
/******************************************************************************/
...
CREATE TABLE CLIENTS (
    ...
    USEEXCESS      FB_BOOLEAN,
    EXCESS         SMALLINT NOT NULL,
    DELTAMODE      SMALLINT,
    ONLYLEADERS    FB_BOOLEAN,
    REQMASK        INTEGER,
...
CREATE TABLE PARAMS (
    ...
    FASTPRINT             FB_BOOLEAN,
    SHOWREGISTER          FB_BOOLEAN,
    NEWWARES              FB_BOOLEAN,
    USEFORMS              FB_BOOLEAN,
    OPERATEFORMS          FB_BOOLEAN,
    OPERATEFORMSSET       FB_BOOLEAN,
    AUTOPRINT             FB_BOOLEAN,
    STARTPAGE             SMALLINT NOT NULL,
    LASTCOMPACT           TIMESTAMP,
    CUMULATIVE            FB_BOOLEAN,
    STARTED               FB_BOOLEAN,
    EXTERNALORDERSEXE     VARCHAR(255) CHARACTER SET WIN1251,
    EXTERNALORDERSPATH    VARCHAR(255) CHARACTER SET WIN1251,
    EXTERNALORDERSCREATE  FB_BOOLEAN,
    RASSLEEP              SMALLINT NOT NULL,
    HTTPNAMECHANGED       FB_BOOLEAN
);
...
CREATE TABLE WAYBILLLIST (
    SERVERID           FB_ID,
    SERVERWAYBILLID    FB_ID,
    FULLCODE           FB_ID,
    CODEFIRMCR         FB_ID,
    SYNONYMCODE        FB_REF,
    SYNONYMFIRMCRCODE  FB_REF,
    SYNONYMNAME        FB_VC250,
    SYNONYMFIRM        FB_VC250,
...
ALTER TABLE NORMATIVES ADD CONSTRAINT PK_NORMATIVES PRIMARY KEY (ID);
...
CREATE INDEX IDX_SYNONYMS_FULLCODE ON SYNONYMS (FULLCODE);
...
DESCRIBE FIELD FULLCODE TABLE CATALOG
'Полный код товара (Name+Form)';

DESCRIBE FIELD SHORTCODE TABLE CATALOG
'Код только наиманования (Name)';

DESCRIBE FIELD NAME TABLE CATALOG
'Наименование товара';

DESCRIBE FIELD FORM TABLE CATALOG
'Форма выпуска';
...
DESCRIBE FIELD ID TABLE NORMATIVES
'Код документа';

DESCRIBE FIELD PARTITION TABLE NORMATIVES
'Раздел. Не может быть равен Null (вместо этого - пустая строка)';

DESCRIBE FIELD NAME TABLE NORMATIVES
'Наименование документа';

DESCRIBE FIELD UPDATED TABLE NORMATIVES
'Признак обновления документа';

DESCRIBE FIELD FILENAME TABLE NORMATIVES
'Имя файла без расширения (.txt)';

DESCRIBE FIELD TITLE TABLE NORMATIVES
'Заголовочная часть документа в rtf виде';

DESCRIBE FIELD DATEDOC TABLE NORMATIVES
'Дата документа';
