...
uses
  ...
  SOAPThroughHTTP, Contnrs, pFIBDataSet, pFIBProps;
...
procedure TReceiveThread.SendFileFromClient;
var
...
procedure TReceiveThread.SendArchivedOrders;
var
    ...
    try
      ...
      for I := 0 to OrdersList.Count-1 do begin
        ...
        if TryStrToInt64(OrdersList[i], ServerOrderId) then
          ...
          Log('Receive', 'Не получилось конвертировать ID заказа : "' + OrdersList[i] + '"');
      end;
    finally
      OrdersList.Free;
    end;
...
procedure TReceiveThread.SendArchivedOrder(ServerOrderId: Int64; adsOrderData : TpFIBDataSet);
const
...
begin
  ...
  try
    if not adsOrderData.IsEmpty then begin
      Log('Receive', Format('Отправка архивного заказа ClientOrderId=%s ServerOrderId=%d по прайсу #%s',
