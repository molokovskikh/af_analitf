    ...
    case Stmt of
      ...
      stmtDESCRIBE: //vic
        begin
          ...
          if (Statement = 'FIELD') or (Statement = 'PROCEDURE') then
          begin
...
(*
      ...
      if (FTokens[0]='DESCRIBE') and (FTokens[1]='PROCEDURE') then //DESCRIBE PROCEDURE SP_DOHOD 'ƒоход планируемый';
        Statement := 'UPDATE RDB$PROCEDURES SET RDB$DESCRIPTION = '+FTokens[3]+' WHERE (RDB$PROCEDURE_NAME = '''+FTokens[2]+''')';
      if (FTokens[0]='DESCRIBE') and (FTokens[1]='PARAMETER') and (FTokens[3]='PROCEDURE') then // DESCRIBE PARAMETER D PROCEDURE USZ_ZN_SDN '1е число соответствующего мес€ца';
...
begin
  ...
  //### тут пришлось добавить проверку -иначе AV на правильном скрипте by Svetlana Bulanova
...
//          (FTokens[FTokens.Count - 1] = ';'))) then {do not localize}
      ...
      if (not InQuote) and (not InDouble) then              //анализ данных внутри строки не нужен by ¬еселов ћихаил
      begin                                                 //by ¬еселов ћихаил
...
//          ' ':                   {do not localize}
    ...
    // Ќа случай, если какой-то ебантей напишет так: CREATE /* */ PROCEDURE ...
    ...
    if ((AnsiUpperCase(NextWord) = 'PROCEDURE') or {do not localize}
      ...
      InSpecial := true;
      //мои изменени€ дл€ правильной работы DROP TRIGGER, DROP PROCEDURE, EXECUTE PROCEDURE
...
//PAVEL <-
    ...
    // 09.01.2005 Kirov Ilya. ќбработка GRANT ... PROCEDURE
    // ->
...
begin
  ...
  if not InInput then LineIndex := ScriptIndex - 1;//pavel  перенесено со строки 599, т.к. в событие OnParse свойство CurrentLine содержало значени€ начала комментари€
  InQuote := false;
  InDouble := false;
  InComment := false;
  Done := false;
...
//### by Svetlana Bulanova закомментировано FCharSet := Database.ConnectParams.CharSet; 
      Inc(i, 2);
    end else
      Inc(i, 1);
  end;
...
begin
  ...
  i := 3;
  DataBase.DBParams.Clear; //### иначе - дублируютс€ by Svetlana Bulanova 
  while i < SQLParser.CurrentTokens.Count - 1 do begin
...
*)
  ...
  raise exception.Create('Unsupported DESCRIBE type');
