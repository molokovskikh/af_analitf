...
{$IFNDEF CLR}
  CLRClasses,
{$ENDIF}
  Classes, SysUtils, MemUtils, CRVio;
...
{$ENDIF}
  ...
  {$ENDIF}
    ...
    ip_addr := {$IFOPT R+}cardinal{$ENDIF}(inet_addr(PAnsiChar(Fhostname)));
    if ip_addr <> Cardinal(INADDR_NONE) then
    ...
    begin
      ...
      h_addr := Marshal.ReadIntPtr(hp.h_addr);
      ip_addr:=
         Marshal.ReadByte(h_addr, 0) +            // byte(hp.h_addr^[0]) +
        (Marshal.ReadByte(h_addr, 1) shl 8) +     // (byte(hp.h_addr^[1]) shl 8) +
        (Marshal.ReadByte(h_addr, 2) shl 16) +    // (byte(hp.h_addr^[2]) shl 16) +
        (Marshal.ReadByte(h_addr, 3) shl 24);     // (byte(hp.h_addr^[3]) shl 24);
      sock_addr.sin_addr := in_addr(ip_addr);
      hp.h_length := 10;
    end;
