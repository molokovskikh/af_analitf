...
{$ELSE}
  Windows,
{$ENDIF}
  Classes, SysUtils, MemUtils, CRVio, CRVioTcp;
...
procedure InitSSLLib;
begin
  EnterCriticalSection(hSSLInit);
  try
    if not LoadedSSLLib then
      LoadSSLLib;
  finally
    LeaveCriticalSection(hSSLInit);
  end;
end;
...
{$ENDIF}
  LIBEAY32DLL := '';
  SSLEAY32DLL := '';
  InitializeCriticalSection(hSSLInit);
...
{$ENDIF}
  DeleteCriticalSection(hSSLInit);
