...
begin
  ...
  for i := 0 to SubStrLen - 1 do begin
    ...
    if PAnsiChar(integer(FBlocks[0]) + SO)^ <> SubStr[i + 1] then begin
      Result := False;
      Exit;
    end;
    Inc(SO);
...
begin
  if FCurPosition > (FActualBlockSize - 1) then
    LoadNextBlock;
  Result := PAnsiChar(integer(FBlocks[0]) + FCurPosition)^;
  Inc(FCurPosition);
end;
...
begin
  ...
  while SI <= (FMaxNumBlock - 1) do begin
    while I <= (FActualBlockSize - 1) do begin
      ...
      if PAnsiChar(integer(FBlocks[SI]) + I)^ <> SubStr[SubIndex] then begin
        SubIndex := 1;
        FoundPos := -1;
        Inc(I);
        Continue;
