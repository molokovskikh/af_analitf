...
type
  ...
  protected
    ...
    procedure ExecuteStatement(SQL: _string; StatementType: integer; var Omit: Boolean; out BreakExec: boolean; Params: TDAParams = nil); virtual;
      ...
      try
        ...
        repeat
          ...
          if not Ready and (Code <> lcString) then
            if FCurrDelimiter = FOwner.FDelimiter{';'} {old mode} then
              Ready := ((FSt = FOwner.FDelimiter) and not IsSpecificSQL(StatementType)) or
          ...
          if Ready then begin
            ...
            if not ((FSt = '/') and (FParser.PrevCol = 0)) or (FCurrDelimiter <> FOwner.FDelimiter{';'}) then
...
procedure TDAScriptProcessor.ExecuteStatement(SQL: _string; StatementType: integer;
...
begin
  ...
  begin
    ...
    if Assigned(FOwner.FBeforeExecute) then
      FOwner.FBeforeExecute(Self, SQL, Omit);
    ...
    if not Omit then begin
      try
        if not (FOwner.UseOptimization and FOwner.FAllowOptimization) then
          FOwner.InternalExecute(SQL, BreakExec, Params)
        else
        begin
