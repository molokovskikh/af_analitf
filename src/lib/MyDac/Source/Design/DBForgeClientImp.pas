...
type
  ...
  public
    constructor Create(ADADesignUtils: TDADesignUtilsClass; ASqlService: {$IFDEF WIN32}IUnknown{$ELSE}TObject{$ENDIF};ADefaultConnectionStr: string); override;
...
procedure TSqlSourceF.Set_Sql({$IFDEF WIN32}const {$ENDIF}Param1: TString);
begin
  SetSqlText(Param1);
end;

procedure TSqlSourceF.GetParameter({$IFDEF WIN32}const {$ENDIF}Index: integer; out Info: CommandParameterInfo);
begin
...
procedure TSqlSourceF.Set_ParameterCount({$IFDEF WIN32}const {$ENDIF}Value: integer);
var
  i, n: integer;
begin
  if Params = nil then
...
procedure TSqlSourceF.SetParameter({$IFDEF WIN32}const {$ENDIF}Index: integer; Info: CommandParameterInfo);
var
  i, j: integer;
  TempParam: TDAParam;
begin
...
constructor TDBForgeService.Create(ADADesignUtils: TDADesignUtilsClass; ASqlService: {$IFDEF WIN32}IUnknown{$ELSE}TObject{$ENDIF}; ADefaultConnectionStr: string);
begin
