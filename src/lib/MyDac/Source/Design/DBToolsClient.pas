...
{$ENDIF}
  ...
  public
    constructor Create(ADADesignUtils: TDADesignUtilsClass;
      ASqlService: {$IFDEF WIN32}IUnknown{$ELSE}TObject{$ENDIF};
  ...
  public
    ...
    function CheckDevTools(const ClassIDs: array of TGUID; const ProviderKey: string; const DADesignUtils: TDADesignUtilsClass;
      out ServiceVersion: int64 {$IFDEF WIN32}; FoundClassID: PGUID = nil{$ENDIF};
      UseNewRegPath: boolean = False): TNeedToCheckDbTools;
...
begin
  ...
  try
    ...
    NeedToCheck := CheckDevTools(ClassIDs, ProviderKey, DADesignUtils,
      ServiceVersion{$IFDEF WIN32}, @ClassId{$ENDIF}, DBToolsServiceClass.UseNewRegPath);
    ...
    try
      try
        ...
        if UseNewRegPath then begin
          ModuleGUID := '';
        {$IFDEF VER12}
          ProviderRegPath := 'SOFTWARE\Devart\dbForge Fusion\Delphi 2009\';
        {$ELSE}
        {$IFDEF VER11}
          ProviderRegPath := 'SOFTWARE\Devart\dbForge Fusion\Delphi 2007\';
        {$ELSE}
          Assert(False);
        {$ENDIF}
        {$ENDIF}
          ...
          if ModuleGUID <> '' then
            if OpenKeyReadOnly(ProviderRegPath + 'Modules\' + ProviderKey)
              and ValueExists('Assembly')
            then begin
              Result := ncIncompatible;
