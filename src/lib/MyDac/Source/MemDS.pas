...
function TMemDataSet.GetFieldXMLValue(Field: TField; FieldDesc: TFieldDesc): WideString;
var
  Buffer: TBytes;
  Blob: TBlob;
  Piece: PPieceHeader;
...
begin
  ...
  {$ENDIF}
    ...
    {$ELSE}
      SetLength(StrValue, Integer(Blob.Size) * 2);
      sbOffset := 0;
      while Piece <> nil do begin
        BinToHex(IntPtr(Integer(Piece) + sizeof(TPieceHeader)),
          PChar(Integer(StrValue) + sbOffset), Piece.Used);
