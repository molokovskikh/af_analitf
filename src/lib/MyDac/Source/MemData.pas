...
{ TData }
  ...
  { Fields }
    ...
    procedure PutFieldData(Field: TFieldDesc; FieldBuf: IntPtr; Source: IntPtr); virtual;
  ...
  TCompareOption = (coCaseInsensitive, coPartialKey, coPartialCompare, coOrdinalCompare);
...
{$ENDIF}
  ...
  { Pieces }
    class procedure AllocPiece(var Piece: PPieceHeader; Size: cardinal);
  ...
  protected
    ...
    function GetPieceSize: cardinal;
    procedure SetPieceSize(Value: cardinal);
  ...
  { Unicode to Ansi conversion methods }
    function TranslatePosition(Position: integer): integer; // Ansi to Unicode
    function GetSizeAnsi: integer; virtual;
  ...
  { Rollback }
    ...
    property PieceSize: cardinal read GetPieceSize write SetPieceSize;
...
begin
  ...
  DestEnd := IntPtr(Integer(Dest) + Count);
  while (Integer(Source) < Integer(SourceEnd)) and
    (Integer(Dest) < Integer(DestEnd))
  do begin
  ...
  if Integer(Dest) > Integer(DestEnd) then // last char was CRLF
    Dest := DestEnd;
  Marshal.WriteByte(Dest, 0);
  Result := Integer(Dest) - Integer(DestStart);
end;
...
begin
  DestStart := Dest;
  SourceEnd := IntPtr(Integer(Source) + Count);
  DestEnd := IntPtr(Integer(Dest) + DestLen);
  while (Integer(Source) < Integer(SourceEnd)) and
    (Integer(Dest) < Integer(DestEnd))
  do begin
    w := Marshal.ReadInt16(Source);
...
begin
  ...
  // Count does not include zero terminator
  DestEnd := IntPtr(Integer(Dest) + Count * 2);
  while (Integer(Source) < Integer(SourceEnd)) and
    (Integer(Dest) < Integer(DestEnd))
  do begin
  ...
  if Integer(Dest) > Integer(DestEnd) then // last char was CRLF_UTF16
    Dest := DestEnd;
  Marshal.WriteInt16(Dest, 0);
  Result := (Integer(Dest) - Integer(DestStart)) div 2;
end;
...
begin
  DestStart := Dest;
  SourceEnd := IntPtr(Integer(Source) + Count * 2);
  DestEnd := IntPtr(Integer(Dest) + DestLen * 2);
  while (Integer(Source) < Integer(SourceEnd)) and
    (Integer(Dest) < Integer(DestEnd))
  do begin
    w := Marshal.ReadInt32(Source);
...
begin
  ...
  {$ENDIF}
    ...
    {$ELSE}
      CopyBuffer(Pointer(Integer(Data) + sizeof(word)), Dest, Marshal.ReadInt16(Data));
    {$ENDIF}
    end;
    dtString:
      if NeedConvertEOL then
...
begin
  ...
  if not Field.HasParent then begin
    FieldBuf := IntPtr(Integer(RecBuf) + Field.Offset);
    NativeBuffer := True;
  end
  else
...
procedure TData.PutFieldData(Field: TFieldDesc; FieldBuf: IntPtr; Source: IntPtr);
var
  Dest: IntPtr;
  D: double;
  Len: integer;
...
{$ENDIF}
  ...
  if not Field.HasParent then begin
    FieldBuf := IntPtr(Integer(RecBuf) + Field.Offset);
    PutFieldData(Field, FieldBuf, Source);
      ...
      case VarType(Value) of
        ...
        varSingle,varDouble{$IFDEF WIN32},varCurrency{$ENDIF}:
          Marshal.WriteInt64(FieldData,
            BitConverter.DoubleToInt64Bits(SetScale(Value, Field.Scale)));
      else
        if VarIsStr(Value) then
          Marshal.WriteInt64(FieldData,
            BitConverter.DoubleToInt64Bits(SetScale(StrToFloat(Value), Field.Scale)))
        else
          raise EConvertError.Create(SCannotConvertType);
      end;
    dtDateTime, dtDate, dtTime: begin
  ...
  for i := 0 to FieldCount - 1 do begin
    ...
    if not Field.HasParent then begin
      case Fields[i].DataType of
        ...
        dtExtVarBytes: begin
          SrcPtr := Marshal.ReadIntPtr(Source, Field.Offset);
          DestPtr := IntPtr(Integer(Dest) + Field.Offset);
          if SrcPtr = nil then
...
//    if FilterNoPartialCompare then;
  ...
  case Node.NodeType of
    ntEqual, ntLike:
      if FilterNoPartialCompare or not VarIsStr(V1) then
        Result := V1 = V2
      else
  ...
  try
        ...
        {$ENDIF}
          ...
          if IsBlobFieldType(FieldDesc.DataType) then begin
            ...
            try
              ...
              St := AnsiString(Marshal.PtrToStringUni(BlobValue));
            finally
              Marshal.FreeHGlobal(BlobValue);
            end;
          end
    ...
    {$IFNDEF FPC}
      ...
      {$IFDEF CLR}
        ...
        Marshal.Copy(IntPtr(Integer(RecBuf) + FieldDesc.Offset), Data, 0, SizeOf(TSQLTimeStamp));
            ...
            try
              case CacheItem.Item.Status of
                isAppended: begin
                  ...
                  if FIndexFields.Count > 0 then
                    MoveDir := CompareRecords(NewCacheRecBuf, IntPtr(Integer(CacheItem.Item) + sizeof(TItemHeader)))
                  else
                    MoveDir := 0;
...
begin
  ...
  RollbackRecBuf := IntPtr(Integer(Item.Rollback) + sizeof(TItemHeader));
  ItemRecBuf := IntPtr(Integer(Item) + sizeof(TItemHeader));
  for i := 0 to FieldCount - 1 do
    if IsBlobFieldType(Fields[i].DataType) then begin
...
function TCRBlobData.TranslatePosition(Position: integer): integer; // Ansi to Unicode
var
  Piece: PPieceHeader;
...
function TCRBlobData.GetSizeAnsi: integer;
var
  Piece: PPieceHeader;
  i: integer;
  p: IntPtr;
...
class procedure TCRBlobData.AllocPiece(var Piece: PPieceHeader; Size: cardinal);
begin
  Assert(Size > 0);
...
procedure TCRBlobData.ReallocPiece(var Piece: PPieceHeader; Size: cardinal);
var
  MemSize: integer;
begin
  if Size = 0 then begin
...
class procedure TBlob.AllocPiece(var Piece: PPieceHeader; Size: cardinal);
begin
...
var
  Remainder, Position, Size, BufLen: cardinal;
  Buffer: TBytes;
  BufferPtr: IntPtr;
{$IFDEF CLR}
  Handle: IntPtr;
...
var
  BufLen, Size, Position, Remainder: cardinal;
  Buffer: TBytes;
  BufferPtr: IntPtr;
{$IFDEF CLR}
  Handle: IntPtr;
...
function TBlob.GetPieceSize: cardinal;
begin
  Result := FData.FPieceSize;
end;

procedure TBlob.SetPieceSize(Value: cardinal);
begin
...
function TCompressedBlobData.CompressFrom(source: IntPtr; const sourceLen: longint): boolean;
var
  CPiece: PPieceHeader;
  CSize: integer;
begin
...
function TCompressedBlobData.Read(Position, Count: cardinal;
