...
{$ENDIF}
  ...
  procedure CopyBufferAnsi(const Source: AnsiString; Dest: IntPtr; Count{Bytes (#0 included)}: cardinal);
...
{$IFDEF CLR}
  ...
  procedure StrLCopy(Dest: PAChar; const Source: PAChar; MaxLen{Chars}: Cardinal);
  function StrLen(const Str: PAChar): Cardinal;
...
{$ENDIF}
  ...
  procedure StrLCopyW(Dest: PWChar; const Source: PWChar; MaxLen{WideChars}: Cardinal);
  function StrLenW(const Str: PWChar): Cardinal;
...
begin
  SetLength(buf, Count);
  CountInt := Convert.ToInt32(Count); // To prevent CLR compiler error
  Encoding.Unicode.GetBytes(Source, 0, (CountInt - 2{#0}) shr 1, buf, 0);
  buf[CountInt - 1] := 0;
  buf[CountInt - 2] := 0;
  Marshal.Copy(buf, 0, Dest, CountInt);
end;
...
{$IFNDEF CLR}
function StrLenW(const Str: PWChar): Cardinal; assembler;
asm
        MOV     EDX,EDI
        MOV     EDI,EAX
        MOV     ECX,0FFFFFFFFH
