...
//  TMyErrorProc = procedure (E: {$IFDEF LITE}Exception{$ELSE}EDAError{$ENDIF}; var Fail: boolean) of object;
  ...
  TMyProtocol = (mpDefault, mpTCP, mpSocket, mpPipe, mpMemory{$IFDEF HAVE_OPENSSL}, mpSSL{$ENDIF});
  ...
  public
    ...
    procedure Assign(Source: TMySQLConnection);
...
{ TMySQLRecordset }
  ...
  public
    class function NormalizeName(const Value: _string; const QuoteNames: boolean = False): _string; overload; override;
    class function NormalizeName(const Value: _string; const LeftQ: _char; const RightQ: _char; const QuoteNames: boolean = False): _string; overload; override;

    class function LeftQuote: _char; override;
    class function RightQuote: _char; override;
    class function IsQuoted(const Value: _string): boolean; override;
    class function QuotesNeeded(Value: _string): boolean; override;
  ...
  { Fields }
    ...
    procedure PutFieldData(Field: TFieldDesc; FieldBuf: IntPtr; Source: IntPtr); override;
...
const
  ...
  function BracketIfNeed(const Value: _string; const Force: boolean = False): _string; overload;
  function BracketIfNeed(const Value: _string; const LeftQ: _char; const RightQ: _char; const Force: boolean = False): _string; overload;
  function Unbracket(const Value: _string): _string;
  function UnbracketIfPossible(const Value: _string): _string; overload;
  function UnbracketIfPossible(const Value: _string; var sl: _string {Result parts in reverse order delimited by slDelimiter}): _string; overload;
  function UnbracketIfPossible(const Value: _string; out DataBase: _string; out ObjName: _string): _string; overload;
  function IsBracketPresent(const Value: _string): boolean;
  ...
  function GenerateTableName(const FieldDesc: TFieldDesc): _string; overload;
  ...
  function ChangeDecimalSeparator(const Value: Variant): AnsiString;
...
{$ELSE}
function EscapeAndQuoteStr(const p: PAnsiChar; Len: integer; const NeedBackslashes: boolean): AnsiString; overload;
  ...
  begin
    ...
    while from <> _end do begin
      if not NeedBackslashes then
        ...
        if SysLocale.FarEast and (from^ in LeadBytes) then begin
          _to^ := '\';
          Inc(_to);
          _to^ := from^;
          Inc(_to);
...
function EscapeAndQuoteStr(const p: AnsiString; Len: integer; const NeedBackslashes: boolean): AnsiString; overload;
begin
  Result := EscapeAndQuoteStr(PAnsiChar(p), Len, NeedBackslashes);
end;
...
{$IFDEF CLR}
function EscapeAndQuoteVar(const Data: variant; const UseUnicode: boolean; const NeedBackslashes: boolean): string;
var
  Blob: TBlob;
  s: string;
  b: TBytes;
...
{$ELSE}
function EscapeAndQuoteVar(const Data: variant; const UseUnicode: boolean{$IFDEF HAVE_COMPRESS}; const SendBlobCompressed: boolean{$ENDIF}; const NeedBackslashes: boolean): AnsiString;
  ...
  begin
    ...
    if not UseUnicode then
      Result := EscapeAndQuoteStr(PAnsiChar(Blob.FirstPiece) + sizeof(TPieceHeader), Blob.FirstPiece.Used, NeedBackslashes)
    else
      if Blob.IsUnicode then
        Result := UTF8Encode(EscapeAndQuoteWideStr(PWideChar(Integer(Blob.FirstPiece) + sizeof(TPieceHeader)), Blob.FirstPiece.Used div 2, NeedBackslashes))
      else
        Result := AnsiToUtf8(string(EscapeAndQuoteStr(PAnsiChar(Blob.FirstPiece) + sizeof(TPieceHeader), Blob.FirstPiece.Used, NeedBackslashes)));
  end;
...
begin
  ...
  {$IFDEF HAVE_COMPRESS}
    ...
    begin
      ...
      begin {uncompr.c} // Does not use SendBlobAsIs for performance reason
        if not UseUnicode then
          Result := EscapeAndQuoteStr(Blob.AsAnsiString, Blob.Size, NeedBackslashes)
        else
          if Blob.IsUnicode then
            Result := UTF8Encode(EscapeAndQuoteWideStr(Blob.AsWideString, Blob.Size div 2, NeedBackslashes))
          else
            Result := AnsiToUTF8(string(EscapeAndQuoteStr(Blob.AsAnsiString, Blob.Size, NeedBackslashes)));
      end;
    end;
  {$ELSE}
    SendBlobAsIs(Blob);
  ...
  if pData.VType = varArray + varByte then
    Result := EscapeAndQuoteStr(pData.VArray.Data, pData.VArray.Bounds[0].ElementCount, NeedBackslashes)
  else
  if UseUnicode then begin
    ...
    begin
      ...
      begin
        s := EscapeAndQuoteStr(TVarData(Data).VPointer, StrLen(PAChar(TVarData(Data).VPointer)), NeedBackslashes);
...
function EscapeAndQuoteTilde(const Value: _string): _string;
var
  Sb: _StringBuilder;
  Len: integer;
  i: integer;
  ...
  function IsQuotePresent(const Value: _string): boolean;
  var
    l: integer;
  begin
    l := Length(Value);
...
begin
  Len := Length(Value);
  Sb := _StringBuilder.Create(Len * 2 + 2);
  try
    Start := 1;
    if IsQuotePresent(Value) and (Len > 1) then begin
      Dec(Len);
      Sb.Append(Value[1]);
      Inc(Start);
    end;
    for i := Start to Len do
      if Value[i] = '`' then begin
        if (Len > i + 1) and (i > 1) and (Value[i + 1] <> '`') and (Value[i - 1] <> '`')  then
          Sb.Append('``')
        else
          Sb.Append(Value[i]);
      end
...
function BracketIfNeed(const Value: _string; const Force: boolean = False): _string;
begin
  Result := BracketIfNeed(Value, _char('`'), _char('`'), Force);
end;

function BracketIfNeed(const Value: _string; const LeftQ: _char; const RightQ: _char; const Force: boolean = False): _string;
var
  i: integer;
begin
  i := Pos('.', Value);
  if i <> 0 then begin
    if (i <> 1) and (i <> Length(Value)) then
      Result := BracketIfNeed(Copy(Value, 1, i - 1)) + '.' + BracketIfNeed(Copy(Value, i + 1, Length(Value) - i))
    else
      Result := LeftQ + Value + RightQ;
  end
  else
    if not TMyTableInfo.IsQuoted(Value) and
      (Force or TMyTableInfo.QuotesNeeded(Value)) then
      Result := LeftQ + Value + RightQ
    else
      Result := Value;
end;

function UnbracketIfPossible(const Value: _string; const NeedSl: boolean; var sl: _string {Result parts in reverse order delimited by slDelimiter}): _string; overload;
var
  DotPos: integer;
  l: integer;
begin
  ...
  if DotPos > 0 then begin
    if DotPos = 1 then
      Result := UnbracketIfPossible(Copy(Value, DotPos + 1, l - DotPos), sl)
    else
      Result := UnbracketIfPossible(Copy(Value, 1, DotPos - 1), sl) + '.' + UnbracketIfPossible(Copy(Value, DotPos + 1, l - DotPos), sl)
  end
  else
  begin
    if TMyTableInfo.IsQuoted(Value) and
      not TMyTableInfo.QuotesNeeded(Value) then
...
begin
  ...
  p := Pos(_string(slDelimiter), Value);
  if p > 0 then begin
    ObjName := Copy(Value, 1, p - 1);
    Database := Copy(Value, p + 1, MaxInt);
  end
  else begin
    ObjName := Value;
    Database := '';
...
{$ENDIF}
  ...
  {$ENDIF}
    while i < Length(Result) do begin
      if Result[i] = AnsiString(DecimalSeparator) then begin
        Result[i] := '.';
        Break;
      end;
      Inc(i);
...
begin
  ...
  {$ENDIF}
  then
    SQL.Append('''0000-00-00 00:00:00''')
  else
  begin
    case DataType of
      dtUnknown, dtBytes, dtVarBytes, dtExtVarBytes, dtBlob: // Unicode not used in any case
        SQL.Append(EscapeAndQuoteVar(Value, False{$IFDEF HAVE_COMPRESS}, SendBlobCompressed{$ENDIF}, NeedBackslashes));
      dtString, dtWideString, dtExtString, dtExtWideString, dtMemo, dtWideMemo:
        SQL.Append(EscapeAndQuoteVar(Value, UseUnicode{$IFDEF HAVE_COMPRESS}, SendBlobCompressed{$ENDIF}, NeedBackslashes));
      dtDateTime: begin
        SQL.Append('''');
        SQL.Append(AnsiString(FormatDateTime('YYYY-MM-DD HH:NN:SS', Value)));
        SQL.Append('''');
      end;
      dtDate: begin
        SQL.Append('''');
        SQL.Append(AnsiString(FormatDateTime('YYYY-MM-DD', Value)));
        SQL.Append('''');
      end;
      dtTime: begin
        ...
        SQL.Append(AnsiString(IntToStr(HourCount)));
        if MinCount >= 10 then
          SQL.Append(':')
        else
          SQL.Append(':0');
        SQL.Append(AnsiString(IntToStr(MinCount)));
        if SecCount >= 10 then
          SQL.Append(':')
        else
          SQL.Append(':0');
        SQL.Append(AnsiString(IntToStr(SecCount)));
        SQL.Append('''');
      end;
      dtFloat, dtCurrency:
        SQL.Append(ChangeDecimalSeparator(Value));
    {$IFNDEF VER6P}
      dtInt64:
      begin
        if VarType(Value) in [$000E, $0014] then begin
          pParamData := @TVarData(Value); // Source
          SQL.Append(IntToStr(PInt64(@pParamData.VInteger)^))
        end
        else
        begin
          i := Value;
          SQL.Append(IntToStr(i));
        end;
      end;
    {$ENDIF}
      dtBCD:
    ...
    {$ENDIF}
        c := Value;
        SQL.Append(ChangeDecimalSeparator(CurrToStr(c)));
      end;
    {$IFDEF VER6P}
    {$IFNDEF FPC}
      dtFmtBCD:
      ...
      {$IFDEF CLR}
        ...
        if (s[1] = '-') and (s[2] = '.') then begin // Delphi bug
          SQL.Append('-0.');
          SQL.Append(s, 2{from 0}, Length(s) - 2);
        end
        else
        begin
    ...
    {$ENDIF}
      ...
      {$IFNDEF VER6P}
        if TVarDataD6(Value).VType = 14 then
          SQL.Append(IntToStr(TVarDataD6(Value).VInt64))
        else
      {$ENDIF}
        SQL.Append(AnsiString(Value));
    end;
  end;
end;
...
begin
  if not FConnected then begin
    ...
    try
      ...
      Check(FMySQLAPI.mysql_real_connect(FMySQL, pServer, PAnsiChar(AnsiString(FUsername)),
        PAnsiChar(AnsiString(FPassword)), PAnsiChar(AnsiString(FDatabase)), FPort, nil, Flags), Component);
      FConnected := True;
...
procedure TMySQLConnection.Assign(Source: TMySQLConnection);
begin
  ...
  FPort := Source.FPort;
  FCompress := Source.FCompress;
  FUseUnicode := Source.FUseUnicode;
  FCharset := Source.FCharset;
  OnError := Source.OnError;
  FProtocol := Source.FProtocol;
  FMySQLAPI := Source.FMySQLAPI;
  FDirect := Source.FDirect;
  FEmbedded := Source.FEmbedded;
  FOptimizedBigInt := Source.FOptimizedBigInt;
  FCheckPrecision := Source.FCheckPrecision;
...
procedure TMySQLCommand.GetSPParams(const Name: _string; out IsFunc: boolean;
  ...
  begin
    ...
    try
      ...
      RecordSet.FCommand.SQL := 'SELECT type, returns, param_list FROM mysql.proc WHERE (name = ''' + UnbracketIfPossible(Name) + ''') AND (LOWER(db) = ''' + AnsiLowerCase(FConnection.FDatabase) + ''')';
      RecordSet.SetProp(prFlatBuffers, False);
      RecordSet.SetProp(prLongStrings, True);
      RecordSet.SetProp(prFetchAll, True);
      RecordSet.Open;
  ...
  begin
    ...
    if IsFunc then
      Result := 'SHOW CREATE FUNCTION ' + Database + '.' + BracketIfNeed(ProcName, FQuoteNames)
    else
      Result := 'SHOW CREATE PROCEDURE ' + Database + '.' + BracketIfNeed(ProcName, FQuoteNames);
  ...
  try
    ...
    while True do begin
      ...
      if Param <> nil then begin
        ...
        if s = 'CHAR' then // (SIZE)?, BINARY?
          Param.SetDataType(dtString)
        else
        if s = 'VARCHAR' then // (SIZE)?, BINARY?
          Param.SetDataType(dtString)
        else
        if s = 'DATE' then
...
begin
  ...
  if IsFunc then begin
    sCall := 'SELECT ' + BracketIfNeed(Name, FQuoteNames) + '(';
    iMin := 1;
  end
  else
  begin
    iMin := 0;
    sCall := 'CALL ' + BracketIfNeed(Name, FQuoteNames) + '(';
  end;
  ...
  {$ENDIF}
    ...
    // TODO: Dbx support (empty param names)
      ...
      pdInputOutput, pdOutput: begin
        ...
        ParamName := '@' + BracketIfNeed(ParamName, FQuoteNames);
        ...
        for i := 0 to FCachedSQL.Count - 1 do begin
          ...
          if i <> FCachedSQL.Count - 1 then begin
            ...
            if Param <> nil then begin
              ...
              if Param <> nil then
                AppendValueToSQL(SQL, Param.GetDataType, Param.Value, Param.GetNull, FConnection.FUseUnicode{$IFDEF HAVE_COMPRESS}, SendBlobCompressed{$ENDIF}, FConnection.FNeedBackslashes, FConnection.ServerPrimaryVer);
            end;
          end;
          Inc(ParamIdx);
        end;
...
class function TMyTableInfo.LeftQuote: _char;
begin
  Result := _char('`');
end;

class function TMyTableInfo.RightQuote: _char;
begin
  Result := _char('`');
end;

class function TMyTableInfo.IsQuoted(const Value: _string): boolean;
var
  l: integer;
begin
  Assert(Pos('.', Value) = 0, 'In func IsQuotedName delimited values not allowed');
...
class function TMyTableInfo.QuotesNeeded(Value: _string): boolean;
var
  i: integer;
  n: integer;
begin
  ...
  Result := False;
  n := Length(Value);
  if (n > 0) and (Value[1] >= '0') and (Value[1] <= '9') then
    Result := True
  else
    for i := 1 to n do
      case Value[i] of
        'a'..'z', 'A'..'Z', '_',{ - see "chapter GRANT and REVOKE syntax"} '@', '#', '0'..'9', '$':;
      else
        begin
          Result := True;
          Exit;
...
class function TMyTableInfo.NormalizeName(const Value: _string; const QuoteNames: boolean = False): _string;
begin
  Result := NormalizeName(Value, _char('`'), _char('`'), QuoteNames);
end;

{$IFDEF VER8}
class function TMyTableInfo.NormalizeName(const Value: _string; const LeftQ: _char; const RightQ: _char; const QuoteNames: boolean = False): _string;
var
  i: integer;
begin // copied from inherited to avoid d8 linker error
  if Value = '' then begin
    Result := '';
    Exit;
  end;
...
class function TMyTableInfo.NormalizeName(const Value: _string; const LeftQ: _char; const RightQ: _char; const QuoteNames: boolean = False): _string;
begin
  Result := inherited NormalizeName(Value, LeftQ, RightQ, QuoteNames);  /// BCB bug with overriding one of some overloaded class functions
end;
{$ENDIF}
...
begin
  try
    ...
    if FCommand.FSQLResult <> nil then
...
procedure TMySQLRecordset.InternalOpen;
begin
  try
    inherited;
  except
...
var
  ...
  begin
    ...
    if ShowStatement then begin
      ...
      Parser.ToBegin;
      if (LowerCase(pField.Name) = 'last_error')
        and (FieldLengthInChars < 80)
      ...
      if (ServerPrimaryVer = 5) and (FCommand.FConnection.ServerReleaseVer > 51) then begin
    ...
    if ExplainStatement4 and (LowerCase(pField.Name) = 'type') then
      FieldLengthInChars := MaxWord - 1; // to prevent range check error at Field.Size := LengthInChars + 1;
  ...
  const
  {$IFDEF VER6P}
    SizeForUnknownSizedFields = MaxWord;
  {$ELSE}
    ...
    if HandlerSyntax or ((not HandlerSyntax) and (Parser.ToLexem(lxSELECT) <> lcEnd)) then begin
      if HandlerSyntax or ((not HandlerSyntax) and (Parser.ToLexem(lxFROM) <> lcEnd)) then
        repeat
          ...
          SQLObjName := UnBracketIfPossible(SQLObjName);
          ...
          begin
            ...
            FTablesInfo[i].TableName := SQLObjName;
          end;
          // i may be -1 for queries like "SELECT count(uid) FROM ALL_TYPES"
          if i <> - 1 then
            FTablesInfo[i].TableNameFull := ObjNameFull;
        until (StLex <> ',');
    end
    else
    begin
  ...
  procedure CreateFieldDescs;
    ...
    var
      Idx: integer;
      IsView: boolean;
      pField_Name, pField_OrgName, pField_Table: _string;
    begin
      ...
      if not IsView then
        ...
        Field.ActualName := pField_Name;

      if pField_Table <> '' then begin
        Idx := AddSQLObjectIfNeed(pField_Table, IsView);
        FTablesInfo[Idx].TableAlias := pField_Table; // pField^.table may contain or object name or alias
      ...
      Field.FIsAutoIncrement := IsFlagSetted(pField.Flags, AUTO_INCREMENT_FLAG);
      Field.Required := not Field.FIsAutoIncrement and (pField._type <> MYSQL_TYPE_TIMESTAMP)
...
begin
  ...
  Marshal.Copy(FNulls, 0, IntPtr(Integer(RecBuf) + DataSize), FFields.Count);
end;
...
procedure TMySQLRecordset.PutFieldData(Field: TFieldDesc; FieldBuf: IntPtr; Source: IntPtr);
var
  Blob: IntPtr;
begin
  case Field.DataType of
  ...
  {$ENDIF}
    ...
    for i := 0 to FieldCnt - 1 do
      if IsNeedFetchBlock(GetCorrectedDataType(FFields[i])) then begin
        FFetchBnd[i].length := IntPtr(Integer(FFetchBlock) + Integer(FFetchBnd[i].buffer));
        FFetchBnd[i].buffer := IntPtr(Integer(FFetchBlock) + Integer(FFetchBnd[i].buffer) + sizeof(UInt));
      end;
  end;
end;
...
var
  ...
  // Create Items
    ...
    if Filtered and not FFetchAll then begin
      if FetchBack then
        InitFetchedItems(IntPtr(Integer(pHBlock) + sizeof(TBlockHeader) + (RowsObtained - 1) * (sizeof(TItemHeader) + RecordSize)), False, FetchBack)
      else
...
{$ENDIF}
  ...
  begin
    if OldpData = nil then begin // Preparing block
      ...
      InitBlock;
      Result := IntPtr(Integer(pHBlock) + sizeof(TBlockHeader) + sizeof(TItemHeader));
    end
    else
      Result := IntPtr(Integer(OldpData) + (RecordSize + sizeof(TItemHeader)));
  end;
  ...
  {$ENDIF}
    ...
    var
      ...
      begin
        ...
        while True do begin
          b := Byte(ValueArr[i]);
          if i = ValueArrOffEnd then
            Break;
        {$IFDEF OVERFLOWCHECKINGON}
          {$OVERFLOWCHECKS OFF}
        {$ENDIF}
          Result := Result * 10 + (b - $30 {Ord('0')});
          Inc(i);
        {$IFDEF OVERFLOWCHECKINGON}
          {$OVERFLOWCHECKS ON}
        {$ENDIF}
  ...
  *)
  ...
  {$ENDIF}
    ...
    begin
      ...
      begin
        ...
        WriteInt32(Integer(ExtStringPtr));
      end;
    end;
    ...
    {$ENDIF}
      ...
      WriteInt32(Integer(Blob.GCHandle));
    end;
    ...
    begin
      ...
      begin
        ...
        API._mysql_fetch_value_to_buff(row, FieldIdx, IntPtr(Integer(HeapBuf) + sizeof(word)), Len);
        WriteInt32(Integer(HeapBuf));
  ...
  function ProcessRowPreparedClient(var pData: IntPtr): boolean;
    ...
    begin
      ...
      pBnd.is_null := IntPtr(Integer(pData) + DataSize + (FieldDesc.FieldNo - 1{numeration from 1}) * sizeof(boolean));

      if not IsNeedFetchBlock(GetCorrectedDataType(FieldDesc)) then begin
        pBnd.buffer := IntPtr(Integer(pData) + FieldDesc.Offset);
        pBnd.length := nil;
      end;
    end;
    ...
    var
      ...
      begin
        if FNullForZeroDate then
          ...
          Marshal.WriteInt64(IntPtr(Integer(pData) + Fields[FieldIdx].Offset), BitConverter.DoubleToInt64Bits(ZeroDate)); // PDateTime(pValue)^ := ZeroDate
      end;
    ...
    begin
      ...
      for FieldIdx := 0 to FieldCount - 1 do begin
        Field := Fields[FieldIdx];
        pValue := IntPtr(Integer(pData) + Field.Offset);
        DataType := Field.DataType; // not corrected!
        if IsNeedFetchBlock(GetCorrectedDataType(Field)) then begin
          pFetchBlockLen := IntPtr(Integer(FFetchBlock) + FetchBlockOffset);
          pFetchBlockValue := IntPtr(Integer(pFetchBlockLen) + sizeof(UInt));
          ...
          case DataType of
            ...
            dtVarBytes: begin
              if GetNull(FieldIdx + 1, pData) then
                ...
                Len := Uint(Marshal.ReadInt32(pFetchBlockLen));
                CopyBuffer(pFetchBlockValue, IntPtr(Integer(pValue) + sizeof(word)), Len);
            ...
            dtExtVarBytes: begin
              if GetNull(FieldIdx + 1, pData) then
                ...
                CopyBuffer(pFetchBlockValue, IntPtr(Integer(ExtBytesPtr) + sizeof(word)), Len);
              ...
              {$ENDIF}
                case mdt.time_type of
                  ...
                  MYSQL_TIMESTAMP_DATETIME: begin
                    if TryEncodeDateTime(mdt.year, mdt.month, mdt.day, mdt.hour, mdt.minute, mdt.second, mdt.second_part, dt) then
                      Marshal.WriteInt64(pValue, BitConverter.DoubleToInt64Bits(dt))
                    else
                      SetZeroDate(FieldIdx);
                  end;
        ...
        if (Param.GetParamType in [pdResult, pdInputOutput, pdOutput]) then begin
          ...
          case Field.DataType of
            ...
            dtExtVarBytes: begin
              ...
              if Param.GetDataType in [dtBlob, dtMemo, dtWideMemo] then begin
                TBlob(Param.GetObject).Clear;
                TBlob(Param.GetObject).Write(0, word(Marshal.ReadInt16(p)), IntPtr(Integer(p) + SizeOf(word)));
              end
              else begin
                SetLength(Data, word(Marshal.ReadInt16(p)));
                Marshal.Copy(IntPtr(Integer(p) + SizeOf(word)), Data, 0, Length(Data));
                Param.Value := Data;
              end;
            end;
          else
...
begin
  ...
  UnbracketIfPossible(ObjName, DataBase, Name);
...
begin
  Assert(FTablesInfo <> nil);
  SQLObjectName := EscapeAndQuoteTilde(SQLObjName);
...
begin
  ...
  if FConnections.Count > 1 then
    raise Exception.Create('multiple transaction'); // upd нормальное сообщение
...
begin
  ...
  if FRecordSet.FCommand.FConnection.ServerPrimaryVer < 5 then begin
    if FRecordSet.FCommand.FConnection.FDatabase = '' then begin
      ...
      SQL := 'SHOW TABLES';
      if Schema <> '' then
        SQL := SQL + ' FROM ' + TMyTableInfo.NormalizeName(Schema);
      FRecordSet.Close;
...
begin
  Schema := Trim(Restrictions.Values['TABLE_SCHEMA']);
  if Schema <> '' then
    Schema := TMyTableInfo.UnQuote(Schema)
  else
...
begin
  ...
  if FRecordSet.FCommand.FConnection.ServerPrimaryVer < 5 then begin
    if TableName = '' then begin
      ...
      SQL := 'SHOW COLUMNS FROM ' + TMyTableInfo.NormalizeName(TableName);
      if Schema <> '' then
        SQL := SQL + ' FROM ' + TMyTableInfo.NormalizeName(Schema);
      FRecordSet.Close;
      FRecordSet.SetSQL(SQL);
...
begin
  ...
  if Schema <> '' then
    ...
    Schema := FRecordSet.FCommand.FConnection.FDatabase;
  TableName := TMyTableInfo.UnQuote(TableName);
...
begin
  ...
  if Schema <> '' then
    UQSchema := TMyTableInfo.UnQuote(Schema)
  else
    UQSchema := FRecordSet.FCommand.FConnection.FDatabase;
  UQProcName := TMyTableInfo.UnQuote(ProcName);
...
begin
  ...
  if TableName = '' then begin
    ...
    SQL := 'SHOW INDEX FROM ' + TMyTableInfo.NormalizeName(TableName);
    if TableSchema <> '' then
      SQL := SQL + ' FROM ' + TMyTableInfo.NormalizeName(TableSchema);
    FRecordSet.Close;
    FRecordSet.SetSQL(SQL);
    FRecordSet.Open;
...
begin
  ...
  for i := 0 to Length(FValues) - 1 do begin
    ...
    AppendValueToSQL(FRowBuffer, Columns[i].DataType, FValues[i],
      ...
      {$IFDEF HAVE_COMPRESS}False, {$ENDIF} NeedBackslashes, FConnection.ServerPrimaryVer);
    FValues[i] := Null;
  end;
end;
