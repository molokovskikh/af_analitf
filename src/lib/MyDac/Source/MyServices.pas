...
type
  ...
  public
    ...
    function QuoteName(const AName: _string): _string; overload; override;
    function QuoteName(const AName: _string; const LeftQuote, RightQuote: _string): _string; reintroduce; overload;
    function UnQuoteName(const AName: _string): _string; override;
...
function TCustomMyDataSetService.QuoteName(const AName: _string): _string;
begin
  Result := QuoteName(AName, FLeftQuote, FRightQuote);
end;

function TCustomMyDataSetService.QuoteName(const AName: _string; const LeftQuote, RightQuote: _string): _string;
begin
  Result := TMyTableInfo.NormalizeName(AName, LeftQuote[1], RightQuote[1], FDataSet.Options.QuoteNames);
end;

function TCustomMyDataSetService.UnQuoteName(const AName: _string): _string;
begin
...
function TCustomMyDumpProcessor.GetTableInfoClass: TTableInfoClass;
begin
  Result := TMyTableInfo;
end;
...
begin
  ...
  if FBackupTables or FBackupData then
...
var
  ...
  var
    ...
    begin
      ...
      UnbracketIfPossible(TableName, Database, ObjName);
          ...
          begin
            if FHexBlob and ((FieldDesc.IsBinary and (FieldDesc.DataType = dtString)) or
              ...
              case FieldDesc.DataType of
                dtString: begin
                  s := Field.{$IFDEF VER12P}AsAnsiString{$ELSE}AsString{$ENDIF};
                {$IFDEF CLR}
              ...
              AppendValueToSQL(CurrentRowSB, FieldDesc.DataType, Value, False,
                False{Connection.Options.UseUnicode}, {$IFDEF HAVE_COMPRESS}False, {$ENDIF}
                NeedBackslashes, IConnection.ServerPrimaryVer);
            end;
          end;
        end;
    ...
    var
      ...
      s: _string;
      CurrentRowSB: AnsiStringBuilder;
      InsHeader: _string;
      ...
      begin
        ...
        try
          ...
          for j := 0 to Length(KeyAndDataFields.DataFieldDescs) - 1 do begin
            FieldName := KeyAndDataFields.DataFieldDescs[j].Name;
            if (Pos('.', FieldName) <> 0) or TMyTableInfo.QuotesNeeded(FieldName) then
              FieldName := '`' + FieldName + '`'; // Can't use BracketIfNeed because of '.' in FieldName (15480)
    ...
    begin
      ...
      if (FBackupTables and (not IsView)) or (FBackupViews and IsView) then begin
        ...
        i := Pos('.', TableName);
        if i <> 0 then begin
          dbName := Unbracket(Copy(TableName, 1, i - 1));
          IConnection.GetProp(prDatabase, v);
          if dbName <> v then begin
            ReplaceTableNameNeed := True;
            ShortTableName := '`' + Unbracket(Copy(TableName, i + 1, Length(TableName) - i)) + '`';
      ...
      if FBackupData and not IsView then begin
        ...
        if RecordCount > 0 then begin
          ...
          if FUseDelayedIns then
            InsHeader := 'INSERT DELAYED INTO ' + InsHeader + ' VALUES'
          else
          ...
          NeedBackslashes := v;
          CurrentRowSB := AnsiStringBuilder.Create;
          try
          ...
          for IsView := False to True do begin
            for i := 0 to TablesList.Count - 1 do begin
              if Boolean({$IFDEF FPC}Pointer{$ENDIF}(TablesList.Objects[i])) <> IsView then
                Continue;
              if (not FBackupTables and not FBackupData) and (not Boolean({$IFDEF FPC}Pointer{$ENDIF}(TablesList.Objects[i]))) then
                Continue;
