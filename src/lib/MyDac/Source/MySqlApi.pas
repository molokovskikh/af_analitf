...
type
  ...
  {$ENDIF} // HAVE_OPENSSL
    ...
    function mysql_options(pmysql: PMYSQL_CON; option: TMySqlOption; var arg: integer): longint; virtual;
...
{$ENDIF}
  ...
  begin
    InternalType := ParamDesc.GetDataType;
    pBnd.buffer := IntPtr(Integer(ParamBlock) + Integer(pBnd.buffer));
    ...
    {$ELSE}
      Len := {$IFDEF UTF8}CLRClasses.{$ENDIF}UnicodeToUtf8(pBnd.buffer, pBnd.buffer_length, PWideChar(ws), Length(ws));
    {$ENDIF}
    ...
    {$ENDIF}
        ...
        {$ENDIF}
          pBnd.buffer := IntPtr(Integer(Blob.FirstPiece) + sizeof(TPieceHeader))
        end;
      end
      else
  ...
  try
    ...
    {$ENDIF}
      ...
      for i := 0 to ParamCnt - 1 do
        FillBindingForParam(Params[i + ParamOffset], bnd[i]{$IFDEF CLR}, IntPtr(Integer(is_null) + i * SizeOf(my_bool)){$ENDIF});
...
begin
  ...
  begin
    ...
    if (fFound) then
    begin
      Windows.VirtualProtectEx(GetCurrentProcess, ppfn, 4, PAGE_EXECUTE_READWRITE, {$IFDEF FPC}@{$ENDIF}written);
...
begin
  if FClientsCount > 0 then begin
    ...
    if (FClientsCount = 0) and IsLibrary then //dbxMda
      FreeMySQLLib;
  end;
end;
