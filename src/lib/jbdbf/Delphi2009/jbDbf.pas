...
{This version works with Delphi 2009!}
...
{This new version works with Delphi 2..7, 2005, 2006/Turbo, 2007 and Delphi2009; Kylix 3 Open Edition}

{history:}
{  18.05.2009 Added codepage information on CreateDB and PackDBF; Tested from D2 to D2009 and Kylix 3 Open Edition;}
{    New demo available; Added support for Delphi2009 but lost support for D1, some types are changed;}
{    Added support to Kylix 3 Open Edition (component and new demo);}
...
{$ELSE} {Delphi}
  uses SysUtils, Classes, Dialogs
    {For Delphi 2009 Compatibility - for to works on ShortString}
    {$IFDEF VER12UP}
      , AnsiStrings
    {$ENDIF}
      , Windows;
...
type
  ...
  TDBFProgress = procedure(Sender: TObject; const Operace: ShortString; Progress: Integer) of object;
  TDBFPassword = procedure(Sender: TObject; var Pass: ShortString) of object;
  ...
  TDBFQuery = procedure(Sender: TObject; const IdxName, IdxField, Key: ShortString;
...
{----------------------------------------------------------------------------}
  ...
  private
    ...
    prgMakeIndexSort : ShortString;
    prgWriteIndexSort : ShortString;
    prgSearchByKey : ShortString;
    {------------------------------------------------------------------------}
    ...
    FDBFIndex       : ShortString;       { Actual index for FIND }
    FDBFPassword    : ShortString;       { Administrator password }
    FDBFFilter      : ShortString;       { Not used}
    ...
    procedure SetPassword(const thepassword: ShortString);
  ...
  public
    ...
    procedure Find(const IdxName, Value: ShortString);{search value by key} virtual;
    procedure Store(const Key:TKey; const Rec: ShortString);{field of record} virtual;
    procedure ELoad(const Key:TKey; var Rec: ShortString);{with conversion} virtual;
    ...
    function  MakeField(posit:Byte;const iname: ShortString; iwhat: AnsiChar; ilen:byte;
                        iplaces:byte;const idxnme: ShortString;
                        Req:TFieldReq;Sort:TSortByIndex):Boolean; virtual;
    function  Cover:Boolean; virtual;
    procedure UnCover; virtual;
    procedure RemoveIndex(const IdxName: ShortString); virtual;
  ...
  published
    ...
    property ByIndex : ShortString read FDBFIndex write FDBFIndex;
...
{----------------------------------------------------------------------------}
function Copy(S : ShortString; Index, Count: Integer): ShortString;
{----------------------------------------------------------------------------}
begin
  Result:=ShortString(System.Copy(String(S), Index, Count));
end;

{----------------------------------------------------------------------------}
function Pos(Substr: ShortString; S: ShortString): Integer;
{----------------------------------------------------------------------------}
begin
  Result:=System.Pos(String(Substr),String(S));
end;

{----------------------------------------------------------------------------}
function StrToInt(const S: ShortString): Integer;
{----------------------------------------------------------------------------}
begin
...
{----------------------------------------------------------------------------}
function TjbDBF.GetPassword: ShortString;
{----------------------------------------------------------------------------}
begin
...
{----------------------------------------------------------------------------}
procedure TjbDBF.SetPassword(const thepassword: ShortString);
{----------------------------------------------------------------------------}
begin
      ...
      try
        ...
        if UpdateIndexes(R) then begin
          try
            ...
            FDBFBuff^[0]:=AnsiChar(' '); { Record not deleted! }         {uncomment /AR/ }
...
function  TjbDBF.MakeField( posit:Byte;
                          const iname: ShortString;
                          iwhat:AnsiChar;
                          ilen:byte;
                          iplaces:byte;
                          const idxnme: ShortString;{filename xxxxxxxxIDX}
                          Req:TFieldReq;
                          Sort:TSortByIndex
                        ):Boolean;
{----------------------------------------------------------------------------}
  ...
  for I:=1 to FDBFCountItems do begin
    ...
    if Trim(FDBFFields[I].Idx)<>'' then begin
      ...
      TempFName:=ExtractFilePath(FDBFName)
       +String(Trim(Copy(FDBFFields[I].Idx,1,8)+'.'+Copy(FDBFFields[I].Idx,9,3)));
          ...
          try
            ...
            try
              ...
              while not Eof(F) do begin
                ...
                try
                  T.Add(' '+String(Copy(S,5,255))+#1+String(IntToStr(L)));
                except
                  on EListError do
              ...
              if not UpdateField then begin
                ELoad(FDBFFields[I].Name,X);
                try
                  T.Add(' '+String(X)+#1+String(IntToStr(R)));
                except
                  on EListError do
                    if T.Duplicates = dupError then begin
                      Warn(msgDuplicateInUnique);
...
begin
  ...
  for I:=1 to FDBFCountItems do begin
    ...
    if Trim(FDBFFields[I].Idx)<>'' then begin
      ...
      {found, format name}
      TempFName:=ExtractFilePath(FDBFName)
       +String(Trim(Copy(FDBFFields[I].Idx,1,8)+AnsiChar('.')+Copy(FDBFFields[I].Idx,9,3)));
      AsSignFile(F,TempFName);
      try
        ReSet(F,1);
        try
      ...
      if FDBFHeader.NumRecs<MaxItems then begin
        ...
        try
          ...
          for I := 0 To FDBFHeader.NumRecs-1 do begin
            ...
            {with position}
            try
              T.Add(' '+String(S)+String(#1)+String(IntToStr(I)));
            except
              on EListError do
...
{----------------------------------------------------------------------------}
procedure TjbDBF.Store(const Key:TKey; const Rec: ShortString);
...
{----------------------------------------------------------------------------}
procedure TjbDBF.ELoad(const Key:TKey; var Rec: ShortString);
...
{----------------------------------------------------------------------------}
procedure TjbDBF.Find(const IdxName, Value{toto je vlastne klic}: ShortString);
{hleda hodnotu podle klice idxname je jmeno sloupce value je hodnota}
{funkcionalitu = <> > < >= <= dodava onquery}
{dearch value by key}
{more give = <> > < >= <= get onquery}
{----------------------------------------------------------------------------}
  function SizeOfKey(const Key: ShortString):Integer;
...
begin
  ...
  {open index}
  AsSignFile(F,ExtractFilePath(FDBFName)+String(IdxName));
  try
    ReSet(F,1);
    try
      B :=SizeOfKey(Value);
      ...
      {nastav prostor na S}
      {make place for it}
      S:='';for I := 1 to Size do S:=S+AnsiChar(' ');
      Cancel:=False;
    ...
    Fatal(Format(msgIdxTableNotFound,[ExtractFilePath(FDBFName)+String(IdxName)]));
  end;
end;
...
{----------------------------------------------------------------------------}
procedure TjbDBF.RemoveIndex(const IdxName: ShortString);
...
begin
  ...
  try
    ...
    try
      ...
      B := 0;
      BlockWrite(F, B, 1);
      SysUtils.DeleteFile(ChangeFileExt(String(IdxName), '.mdx'));
    finally
      CloseFile(F);
    end;
  except
  ...
  try
    ...
    try
      ...
      with T do begin
        ...
        {record no}
        NumberOf:=No;
        S:=ShortString(ExtractFileExt(FName));
        if Length(S)<=3 then
...
try
  ...
  iLenTot:=0;
  FileName := FDBFName;
  sFileNew := System.Copy(FDBFName,1,Length(FDBFName)-Length(ExtractFileExt(FDBFName)))+'_temp_pack.dbf';
       ...
       begin
         ...
         begin
           ...
           begin
             ...
             DBFNew.Store(sField,ShortString(Load(sField)));
           end;
           DBFNew.NewRecord;
         end;
         GotoNext;
